...
/// A type that can be used to index into a `serde_json_2::Value`.
///
    ...
    I: Index,
{
    ...
    /// Index into a `serde_json_2::Value` using the syntax `value[0]` or
    /// `value["k"]`.
    ///
...
where
    I: Index,
{
    /// Write into a `serde_json_2::Value` using the syntax `value[0] = ...` or
    /// `value["k"] = ...`.
    ///

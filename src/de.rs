...
where
    R: read::Read<'de>,
{
    /// Create a JSON deserializer from one of the possible serde_json_2 input
    /// sources.
    ///
...
impl<'de, R: Read<'de>> Deserializer<R> {
    ...
    /// impls.
    ///
    /// *This method is only available if serde_json_2 is built with the
    ...
    ///     }
    ///
    ///     let mut deserializer = serde_json_2::Deserializer::from_str(&json);
...
impl<'de, 'a, R: Read<'de>> de::Deserializer<'de> for &'a mut Deserializer<R> {
    ...
    ///
    /// The behavior of serde_json_2 is specified to fail on non-UTF-8 strings
    ...
    ///
    /// fn look_at_bytes() -> Result<(), serde_json_2::Error> {
    ///     let json_data = b"\"some bytes: \xe5\x00\xe5\"";
    ///     let bytes: ByteBuf = serde_json_2::from_slice(json_data)?;
    ///
...
///
/// ```
/// use serde_json_2::{Deserializer, Value};
///
/// fn main() {
...
where
    ...
    T: de::Deserialize<'de>,
{
    /// Create a JSON stream deserializer from one of the possible serde_json_2
    /// input sources.
    ///
    /// Typically it is more convenient to use one of these methods instead:
    ///
    ...
    /// let data = b"[0] [1] [";
    ///
    /// let de = serde_json_2::Deserializer::from_slice(data);
...
/// without being buffered in memory by serde_json_2.
///
/// When reading from a source against which short reads are not efficient, such
/// as a [`File`], you will want to apply your own buffering because serde_json_2
...
///     let u = serde_json_2::from_reader(reader)?;
///
///     // Return the `User`.
///     Ok(u)
/// }
...
///     let mut de = serde_json_2::Deserializer::from_reader(tcp_stream);
...
///         }";
///
///     let u: User = serde_json_2::from_slice(j).unwrap();
///     println!("{:#?}", u);
/// }
/// ```
///
...
///         }";
///
///     let u: User = serde_json_2::from_str(j).unwrap();
///     println!("{:#?}", u);
/// }
/// ```
///

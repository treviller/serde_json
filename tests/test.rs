...
#[cfg(feature = "raw_value")]
use serde_json_2::value::RawValue;
use serde_json_2::{
...
fn test_value_as_f64() {
    let v = serde_json_2::from_str::<Value>("1e1000");
...
fn test_roundtrip_f64() {
    for &float in &[
        ...
        let json = serde_json_2::to_string(&float).unwrap();
        let output: f64 = serde_json_2::from_str(&json).unwrap();
...
fn test_roundtrip_f32() {
    ...
    let json = serde_json_2::to_string(&float).unwrap();
    let output: f32 = serde_json_2::from_str(&json).unwrap();
    assert_eq!(float, output);
}
...
fn test_malicious_number() {
    #[derive(Serialize)]
    #[serde(rename = "$serde_json_2::private::Number")]
    struct S {
        #[serde(rename = "$serde_json_2::private::Number")]
        f: &'static str,
    }

    let actual = serde_json_2::to_value(&S { f: "not a number" })
        .unwrap_err()
        .to_string();
...
fn test_deny_float_key() {
    ...
    assert!(serde_json_2::to_value(map).is_err());
}
...
fn null_invalid_type() {
    let err = serde_json_2::from_str::<String>("null").unwrap_err();
    assert_eq!(
        format!("{}", err),
...
fn test_borrowed_raw_value() {
    ...
    let wrapper_from_str: Wrapper =
        serde_json_2::from_str(r#"{"a": 1, "b": {"foo": 2}, "c": 3}"#).unwrap();
    assert_eq!(r#"{"foo": 2}"#, wrapper_from_str.b.get());

    let wrapper_to_string = serde_json_2::to_string(&wrapper_from_str).unwrap();
    assert_eq!(r#"{"a":1,"b":{"foo": 2},"c":3}"#, wrapper_to_string);

    let wrapper_to_value = serde_json_2::to_value(&wrapper_from_str).unwrap();
    assert_eq!(json!({"a": 1, "b": {"foo": 2}, "c": 3}), wrapper_to_value);

    let array_from_str: Vec<&RawValue> =
        serde_json_2::from_str(r#"["a", 42, {"foo": "bar"}, null]"#).unwrap();
    ...
    let array_to_string = serde_json_2::to_string(&array_from_str).unwrap();
...
fn test_raw_value_in_map_key() {
    ...
    let map_from_str: HashMap<&RawMapKey, &RawValue> =
        serde_json_2::from_str(r#" {"\\k":"\\v"} "#).unwrap();
...
fn test_boxed_raw_value() {
    ...
    let wrapper_from_reader: Wrapper =
        serde_json_2::from_reader(br#"{"a": 1, "b": {"foo": 2}, "c": 3}"#.as_ref()).unwrap();
    assert_eq!(r#"{"foo": 2}"#, wrapper_from_reader.b.get());

    let wrapper_from_value: Wrapper =
        serde_json_2::from_value(json!({"a": 1, "b": {"foo": 2}, "c": 3})).unwrap();
    ...
    let array_from_reader: Vec<Box<RawValue>> =
        serde_json_2::from_reader(br#"["a", 42, {"foo": "bar"}, null]"#.as_ref()).unwrap();
...
fn test_raw_invalid_utf8() {
    ...
    let value_err = serde_json_2::from_slice::<Value>(j).unwrap_err();
    let raw_value_err = serde_json_2::from_slice::<Box<RawValue>>(j).unwrap_err();
...
fn test_serialize_unsized_value_to_raw_value() {
    assert_eq!(
        serde_json_2::value::to_raw_value("foobar").unwrap().get(),
        r#""foobar""#,
    );
}
...
fn hash_positive_and_negative_zero() {
    ...
    let k1 = serde_json_2::from_str::<Number>("0.0").unwrap();
    let k2 = serde_json_2::from_str::<Number>("-0.0").unwrap();
